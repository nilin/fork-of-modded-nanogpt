...
class Muon(torch.optim.Optimizer):
    def __init__(self, params, lr=0.02, mu=0.95):
...
print0(code)
print0("="*100)
print0(f"script={Path(sys.argv[0]).name}")
print0(f"torch_version={torch.version.__version__}")
print0(f"cuda_version={torch.version.cuda}")
print0(f"device_name={torch.cuda.get_device_name(device)}")
print0(f"world_size={dist.get_world_size()}")
print0(f"seed={SEED}")
print0(f"contra_muon={CONTRA_MUON}")
print0(f"opnorm_grad_subtraction={CONTRA_MUON / 2}")
print0(f"mu={MU}")
print0(f"muon_lr={MUON_LR}")
print0(f"target_uw={TARGET_UW}")
print0("="*100)
...
# Skylight-001: NorMuon-lite (per-row variance) + u/w-floor=0.35 + lr=0.0375.
